...
static void
ed25519_hash_keccak_init(ed25519_hash_context *ctx) {
	cryptonite_keccak_init(ctx -> sc, 512);
}

static void
ed25519_hash_keccak_update(ed25519_hash_context *ctx, const uint8_t *in, size_t inlen) {
	cryptonite_keccak_update(ctx -> sc, in, inlen);
}

static void
ed25519_hash_keccak_final(ed25519_hash_context *ctx, uint8_t *hash) {
	cryptonite_keccak_finalize(ctx -> sc, 512, hash);
}

static void
ed25519_hash_keccak(uint8_t *hash, const uint8_t *in, size_t inlen) {
